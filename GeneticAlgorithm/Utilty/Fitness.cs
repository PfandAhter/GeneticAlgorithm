...
namespace GeneticAlgorithm
{
    public class Fitness
    {
        ...
        {
            int xBits = (int)Math.Ceiling(Math.Log(xLength * precision,2));
            int yBits = (int)Math.Ceiling(Math.Log(yLength * precision,2));
        ...
        public double getActualXValue(bool[] gen)
        {
            ...
            return xMin + ((binaryEncoder.BoolArrayToDouble(gen)) / ((2 ^ xBits) - 1)) * (xMax - (xMin));
        }
        public double getActualYValue(bool[] gen)
        {
            int yBits = requiredBitsNumber().Item2;
            return yMin + ((binaryEncoder.BoolArrayToDouble(gen)) / ((2 ^ yBits) - 1)) * (yMax - (yMin));
        }
    }
}
